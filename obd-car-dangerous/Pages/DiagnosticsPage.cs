...
namespace obd_car_dangerous.Pages
{
    ...
    internal sealed class DiagnosticsPage : PageBase
    {
        ...
        public override string Title => "Vehicle Health";
        ...
        {
            const float pad = 24f;
            float top = DrawHeader(g, AppState.Connection.IsConnected ? null : "adapter offline");
        ...
        private void DrawScoreCard(Graphics g, RectangleF bounds)
        {
            ...
            Draw.TextCentered(g, "Overall Health", Draw.Font(26), Draw.Alpha(Color.White, 220),
            ...
            if (scanProgress >= 0f)
            {
                ...
                Draw.TextIn(g, $"Scanning {systems[Math.Clamp(scanSystem, 0, systems.Count - 1)].Name}...",
            ...
            {
                Draw.TextIn(g, $"Last Check   {AppState.LastScan:yyyy-MM-dd HH:mm}", Draw.Font(19), Draw.Alpha(Color.White, 200),
                    new RectangleF(footer.X, footer.Y, footer.Width, 30), StringAlignment.Near, StringAlignment.Center, false);

                var button = new RectangleF(footer.X, footer.Y + 40, footer.Width, 62);
                DrawButton(g, button, "Full Scan", Color.FromArgb(0, 132, 255), Color.White, StartScan, "diag-scan", 16f);
            }
        }
        ...
        private void StartScan()
        {
            ...
            {
                OpenModal("Adapter offline", "Connect to the OBD2 adapter before running a full system scan.",
                    "Open connection", Theme.Accent, () => Shell.Navigate("settings", "connection"));
                return;
            }
        ...
        private void DrawSystems(Graphics g, RectangleF bounds)
        {
            ...
            for (int i = 0; i < systems.Count; i++)
            {
                ...
                Draw.TextIn(g, system.Name, Draw.Font(25, FontStyle.Bold), Theme.Text,
                    new RectangleF(iconBox.Right + 20, row.Y + 12, row.Width * 0.42f, row.Height / 2f),
                    StringAlignment.Near, StringAlignment.Center, false);

                Draw.TextIn(g, scanning && scanProgress < 1f ? "Reading module..." : system.Detail, Draw.Font(17), Theme.TextSoft,
                    new RectangleF(iconBox.Right + 20, row.Y + row.Height / 2f - 6, row.Width * 0.5f, row.Height / 2f),
                    StringAlignment.Near, StringAlignment.Center, false);

                Draw.TextIn(g, system.Status, Draw.Font(23, FontStyle.Bold), color,
